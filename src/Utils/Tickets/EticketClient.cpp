...
#define OST_ETICKET_URL ""
    ...
    bool EnsureFetched(AppId_t appId, std::span<const uint8_t> nonce, uint64_t existingSteamId, CachedTickets& out) {
        ...
        // The backend may return steam_id as a bare JSON integer rather than a
        // string, which ExtractStringField() cannot see.
        if (!ExtractUInt64Field(r.body, "steam_id", fetched.steamId)) {
            fetched.steamId = 0;
            LOG_IPC_WARN("EticketClient: no usable steam_id in response, ignoring");
        }
