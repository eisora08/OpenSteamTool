...
void Apply(const PipeContext& ctx) {
    ...
    if (injectDlls.empty()) {
        ...
        if (arch == OSTPlatform::RemoteProcess::Architecture::X86)
            autoPath = Config::GetInjectLibraryX86();
        else if (arch == OSTPlatform::RemoteProcess::Architecture::X64)
            autoPath = Config::GetInjectLibraryX64();

        if (!autoPath.empty() && ClaimInjection({ctx.process, autoPath})) {
            const std::filesystem::path path(autoPath);
            ...
            if (status == OSTPlatform::RemoteProcess::InjectStatus::Ok) {
                LOG_INJECT_INFO("auto-injected pid={} appid={} arch={} dll=\"{}\"",
                                ctx.process.pid, ctx.appId,
                                arch == OSTPlatform::RemoteProcess::Architecture::X86 ? "x86" : "x64",
                                path.filename().string());
            } else {
                LOG_INJECT_WARN("auto-inject failed pid={} appid={} status={} dll=\"{}\"",
                                ctx.process.pid, ctx.appId,
